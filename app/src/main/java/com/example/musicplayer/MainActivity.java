...
public class MainActivity extends AppCompatActivity implements RecyclerViewClickInterface, Serializable {
    ...
    public void getAllAudioFromDevice(final Context context) {
        ...
        String[] projection = {MediaStore.Audio.AudioColumns.DATA, MediaStore.Audio.AudioColumns.ALBUM, MediaStore.Audio.ArtistColumns.ARTIST, MediaStore.Audio.Albums.ALBUM_ID};
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                song.setPath(path);
                song.setUrl(songURL);
                Bitmap bm = getAlbumart(cursor.getLong(cursor.getColumnIndexOrThrow(MediaStore.Audio.Albums.ALBUM_ID)));
                song.setBitmap(bm);
